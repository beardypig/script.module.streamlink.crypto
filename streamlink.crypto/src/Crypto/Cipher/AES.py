...
from pyrijndael import rijndael
...
def new(key, mode=MODE_ECB, IV=None, counter=None, segment_size=None):
    ...
    **********
    ...
    >>> crypt += cipher.final()
    >>> decipher = AES.new('0123456789012345')
    ...
    >>> from Crypto.Util.util import Counter
    >>> key = '2b7e151628aed2a6abf7158809cf4f3c'.decode('hex')
...
class AES(BlockCipher):
    ...
    def __init__(self, key, mode, IV, counter, segment_size):
        args = {'block_size': 16}
        self.blocksize = 16
        super(AES, self).__init__(key, mode, IV, counter, rijndael, segment_size, **args)
